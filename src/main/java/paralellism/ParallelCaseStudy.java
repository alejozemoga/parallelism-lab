...
public class ParallelCaseStudy {
    private static final String DIR = System.getProperty("user.dir") + "/lab/";

    public static void main(String[] args) throws IOException {

        Files.createDirectories(Paths.get(DIR));

        ParallelCaseStudy obj = new ParallelCaseStudy();

        List<Employee> employees = obj.generateEmployee(10000);

        // normal, sequential
        employees.stream().forEach(ParallelCaseStudy::save); 		// 27s-29s

        // parallel
        //employees.parallelStream().forEach(ParallelCaseStudy::save); // 7s-8s
